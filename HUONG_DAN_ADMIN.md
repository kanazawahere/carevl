...
Tài liệu này dành cho bộ phận HQ/Admin vận hành hệ thống CareVL ở cấp quản trị.
...
Admin là bộ phận quản trị hệ thống ở cấp HQ, chịu trách nhiệm:
...
- HQ/Admin merge dữ liệu từ các branch trạm về `main`
...
- File gom snapshot toàn hệ thống: `admin-aggregate-data.bat`

Quy trình sử dụng đề xuất:

1. Sửa `config/stations.csv`
2. Chạy `admin-check-stations.bat`
3. Nếu hợp lệ, chạy `admin-build-stations.bat`
4. Nếu cần bàn giao hoặc theo dõi onboarding, chạy `admin-export-onboarding.bat`
5. Nếu cần gom snapshot dữ liệu toàn hệ thống theo ngày, chạy `admin-aggregate-data.bat`
...
- Xác nhận HQ nhìn thấy branch và thay đổi mới
...
- Dữ liệu có nằm trong đúng thư mục ngày tháng
- Không có thay đổi bất thường ở file cấu hình
- Không có thay đổi không liên quan đến dữ liệu

Nếu branch chỉ chứa thay đổi JSON dữ liệu đúng quy ước thì có thể merge theo quy trình thường.
...
### Tình huống 4. Trạm push dữ liệu lên nhưng HQ chưa thấy trong báo cáo tổng hợp
...
- Chạy bước tổng hợp sau merge nếu quy trình yêu cầu
...
- Xử lý thủ công ở cấp HQ

Nguyên tắc:

- Trạm không tự ý sửa lịch sử Git
- Conflict tổng hợp là trách nhiệm của HQ/Admin
